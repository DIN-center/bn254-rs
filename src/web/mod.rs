...
/// Start the web service with a custom data directory, bind address, port, optional mnemonic, and optional salt
///
/// This function:
/// 1. Initializes the key store from the specified data directory, mnemonic, or derivation salt
...
/// * `salt` - Optional secret salt for on-demand key derivation
///
/// # Errors
///
/// Returns an error if:
...
/// - Salt mode: Derives keys on-demand from EOA + salt (highest priority)
/// - Mnemonic mode: Derives 26 keys from HD wallet
/// - Static mode: Loads keys from data directory
pub async fn start_server(
    ...
    // Initialize store based on salt, mnemonic, or data directory (in priority order)
    let store = if let Some(secret_salt) = salt {
        info!("Initializing store in key derivation mode (EOA + salt)");
        match store::Store::with_derivation(secret_salt) {
            Ok(store) => store,
            Err(e) => {
                error!("Failed to initialize derivation mode: {}", e);
