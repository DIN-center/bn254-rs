//! # Key Store Module V4
//!
//! This module manages BLS key pairs with multiple initialization modes:
//! 1. **Static pool mode**: Load from pre-generated BLS key pool (key_0 through key_49)
//! 2. **EOA mapping mode**: Map EOA addresses to keys from the pool
//! 3. **Derivation mode**: Derive keys on-the-fly from EOA + secret salt
//!
//! ## Workflow - Static Pool
//! ```bash
//! bn254-rs --data-dir /path/to/data
//! ```
//!
//! ## Workflow - Key Derivation
//! ```bash
//! bn254-rs --salt "my-secret-salt" --derive-keys
//! ```
//!
//! ## EOA Keymap Format
//! ```json
...
use crate::keygen::derive_key_from_eoa;
...
#[derive(Debug, Clone)]
pub enum StoreMode {
    /// Static mode: keys are pre-loaded from a pool
    Static,
    /// Derivation mode: keys are derived on-demand from EOA + salt
    Derivation { secret_salt: String },
}

/// In-memory store for operator key pairs
pub struct Store {
    /// Map from EOA address to KeyPair (for static mode)
    players: HashMap<String, KeyPair>,
    /// Map from key_N format to KeyPair (for static mode)
    key_index: HashMap<String, KeyPair>,
    /// Operation mode
    mode: StoreMode,
    /// Cache for derived keys (only used in derivation mode)
    derived_cache: RwLock<HashMap<String, KeyPair>>,
}

impl Store {
    /// Create a new store in derivation mode
    ///
    /// Keys will be derived on-demand from EOA addresses + secret salt
    ///
    /// ## Arguments
    /// * `secret_salt` - Secret salt for key derivation (keep this secure!)
    pub fn with_derivation(secret_salt: String) -> Result<Self> {
        info!("Initializing key store in derivation mode");
        info!("Keys will be derived on-demand from EOA + salt");

        Ok(Self {
            players: HashMap::new(),
            key_index: HashMap::new(),
            mode: StoreMode::Derivation { secret_salt },
    ...
    pub fn get_key_pair(&self, identifier: &str) -> Option<KeyPair> {
        ...
        // In derivation mode, derive the key on-demand
        if let StoreMode::Derivation { ref secret_salt } = self.mode {
            // Check cache first
            {
                let cache = self.derived_cache.read().ok()?;
                if let Some(cached_key) = cache.get(identifier) {
                    debug!(identifier = %identifier, "Key pair found in derivation cache");
                    return Some(cached_key.clone());
                }
            }

            // Derive the key
            debug!(identifier = %identifier, "Deriving key pair from EOA + salt");
            match derive_key_from_eoa(identifier, secret_salt) {
                Ok(key_pair) => {
                    // Cache the derived key
                    if let Ok(mut cache) = self.derived_cache.write() {
                        cache.insert(identifier.to_string(), key_pair.clone());
                    }
                    debug!(identifier = %identifier, "Successfully derived and cached key pair");
                    return Some(key_pair);
                }
                Err(e) => {
                    warn!(identifier = %identifier, error = %e, "Failed to derive key pair");
                    return None;
                }
            }
        }

        // Static mode: lookup in pre-loaded keys
        ...
        let key_pair = self.players.get(identifier).cloned();
