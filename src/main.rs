...
struct Args {
    ...
    /// Secret salt for key derivation mode
    ///
    /// When provided, the service will derive BN254 keys on-demand from EOA addresses + this salt.
    /// This mode does NOT use pre-generated keys from the pool.
    ///
    /// SECURITY WARNING: Keep this salt secret! Anyone with the salt can derive your keys.
    /// Use a high-entropy random string (minimum 32 bytes recommended).
    ///
    /// Example:
    /// --salt "my-super-secret-random-salt-$(openssl rand -hex 32)"
    #[arg(short, long)]
    salt: Option<String>,
}
...
async fn main() {
    ...
    if args.salt.is_some() {
        tracing::info!("Using on-demand key derivation mode (EOA + salt)");
        tracing::warn!("Keys will be derived dynamically - no pre-generated pool used");
    ...
    // This will:
    // 1. Load the key store from the specified data directory, derive from mnemonic, or use salt derivation
    // 2. Start the HTTP server on the specified address and port
    // 3. Set up all API routes
    if let Err(e) = web::start_server(&args.data_dir, &args.host, args.port, args.mnemonic, args.salt).await {
