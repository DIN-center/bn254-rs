...
/// Converts a G1Json structure to a G1Affine point.
/// 
/// # Arguments
...
/// # Returns
/// A G1Affine point
fn g1_from_json(p: &G1Json) -> G1Affine {
    G1Affine::new_unchecked(parse_decimal_fq(&p.x), parse_decimal_fq(&p.y))
}

/// Converts a G2Json structure to a G2Affine point.
/// 
...
/// # Returns
/// A G2Affine point
fn g2_from_json(p: &G2Json) -> G2Affine {
...
fn test_operator_points() {
    ...
    for (i, op) in operators.iter().enumerate() {
        ...
        println!("  G2 is_on_curve: {}", g2.is_on_curve());

        assert!(g1.is_on_curve(), "G1 point {} not on curve", i);
        assert!(g2.is_on_curve(), "G2 point {} not on curve", i);
        assert!(!g1.infinity, "G1 point {} is at infinity", i);
        assert!(!g2.infinity, "G2 point {} is at infinity", i);

        // Verify the pairing check: e(g1, g2) * e(-g1, g2) = 1
        // This is a fundamental property of BLS signatures
        let g1_neg = -g1;
        let e1 = Bn254::pairing(g1, g2).0;
        let e2 = Bn254::pairing(g1_neg, g2).0;
        let result = e1 * e2;

        assert_eq!(
            result,
            <Bn254 as Pairing>::TargetField::one(),
            "Pairing check failed at {}",
            i
        );
    }
}
